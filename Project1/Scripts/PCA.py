...
annualIncome = conData[:,0];
y = np.array(annualIncome) #KEEP CLASSES FOR LATER PLOTS
age = conData[:,1];
...
classNames = sorted(set(["<$10K","$10K to $15K","$15K to $20K","$20K to $25K","$25K to $30K","$30K to $40K","$40K to $50K","$50K to $75K"]))       
...
for j in range(4):
    ...
    # select indices belonging to class c:
        ...
        xlabel('PC{0}'.format(i+1))
        ylabel('PC{0}'.format(j+1))
        plt.savefig("SKRT"+str(i+1)+"vs"+str(j+1)+".png",dpi=350)
        
        
#nameString = (Z[class_mask,i], Z[class_mask,j], 'o', alpha=.3)
#nameString.plot()
