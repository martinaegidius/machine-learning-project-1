...
# import data 
filename = 'filtered_data_K.mat';

mat_data = loadmat(filename)
X = mat_data['attributeValues']
#X = mat_data['attributeValues'][0:30,:]
y = mat_data['classValues']#.squeeze()
#y = mat_data['classValues'].squeeze()[0:30]
mat_data = loadmat('filtered_data.mat')
...
n_hidden_units = [1,2,3,4,5,6] # number of hidden units
...
for i in range(len(n_hidden_units)):
    model.append(lambda: torch.nn.Sequential(
...
for i in range(len(model)):
    ...
    print('Number of hidden units:' + str(n_hidden_units[i]))
